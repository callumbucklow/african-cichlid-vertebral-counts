...
mrca_boxplot <- data.frame(
  ...
  total_count = c(3.3162, 3.389095, 3.429392, 3.454106),
  sd_upper = c(3.3162+0.0317, 3.389095+0.03209378, 3.429392+0.01755014, 3.454106+0.06730421),
  sd_lower = c(3.3162-0.0317, 3.389095-0.03209378, 3.429392-0.01755014, 3.454106-0.06730421)
...
#approximate ±1 vertebra in ln scale
#intraspecific_band <- data.frame(
  #water_system = c("RV", "LV", "LM", "LT"),
  #median = c(3.342846, 3.38835, 3.430447, 3.453824),
  #lower = c(log(exp(3.342846) - 1), log(exp(3.38835) - 1), log(exp(3.430447) - 1), log(exp(3.453824) - 1)),
  #upper = c(log(exp(3.342846) + 1), log(exp(3.38835) + 1), log(exp(3.430447) + 1), log(exp(3.453824) + 1))
#)

#intraspecific_band$xpos <- as.numeric(factor(intraspecific_band$water_system,
                                             #levels = c("LV", "LM", "LT", "RV")))
...
b2 <- ggplot(data = vertebral_counts_no_missing, aes(x = water_system, y = total_count))+
  ...
  #SD for root estimates
  geom_errorbar(data = mrca_boxplot, aes(x = water_system, ymin = sd_lower, ymax = sd_upper),
...
ggsave("Figures/Paper/total_count_plot.jpg", b2, dpi=1200, width = 10, height = 10, units = "cm")
```
...
LT <- data.frame(
  time = LT_time,
  variance = 1.5 * LT_time^1.1,   #slow, long-term growth, basically linear BM
  lake = "LT"
)
...
ggsave("Figures/Paper/discussion_rates_morpho_occupation.jpg", horrow_show, dpi=1200, width = 10, height = 10, units = "cm")
```
