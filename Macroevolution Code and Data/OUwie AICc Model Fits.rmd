...
#calculate the combined AICc weights
combined_aicc_results_w <- combined_aicc_results %>%
  group_by(tree_index) %>% #pseudo-replicate
  mutate(
...
#compare between regimes
regime_support <- combined_aicc_results_w %>%
  group_by(tree_index, regime) %>%
  summarise(regime_weight = sum(weight), .groups = "drop") %>%
  group_by(regime) %>%
  summarise(
    median_support = median(regime_weight),
    percentage_support = median_support * 100 #median AICcW is probably fine
  )
