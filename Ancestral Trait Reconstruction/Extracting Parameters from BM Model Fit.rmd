...
file_paths <- paste0("Node and Parameter Estimates/ln[TC]/ln_tc_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_model_params.txt")
...
#access the file paths
file_paths <- paste0("Node and Parameter Estimates/ln[TC]/ln_tc_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")
...
files_tc <- paste0("Node and Parameter Estimates/ln[TC]/ln_tc_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")
...
#import tree:
all_species_tree <- read.tree("Species Tree and Data/all_species_tree_piscivore_pruned.txt")

#substitue _ for space in tip labels:
all_species_tree$tip.label <- gsub("_", " ", all_species_tree$tip.label)

#import total_count data
all_species_total_count <- read.csv("Species Tree and Data/all_species_matrix_data.csv")
...
pdf("contmap_tc_all_species_MCMC.pdf", width=8.4, height=11)
...
#access the file paths
file_paths <- paste0("Node and Parameter Estimates/ln[PC]-ln[C]/ln_pc_ln_c_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_model_params.txt")
...
```{r Importing Node State Estimates for ln[PC]-ln[C]}

#define seed values used in the model fitting (part of filename)
seed_values <- c(1931, 1971, 1970, 1996, 2016)

files_tc <- paste0("Node and Parameter Estimates/ln[PC]-ln[C]/ln_pc_ln_c_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")

# Read all files into a list
ln.pc.ln.c.node.states <- lapply(files_tc, read.table, header = TRUE)

# If you want to assign each dataframe to its own variable, you can do so like this:
names(ln.pc.ln.c.node.states) <- paste0("ln.pc.ln.c.node.states.", 1:5, ".param")
list2env(ln.pc.ln.c.node.states, envir = .GlobalEnv)

#filter first 10% of estimated node states which corresponds to burnin 
ln.pc.ln.c.node.states.burnin.filtered <- lapply(ln.pc.ln.c.node.states, function(df) {
  df %>% filter(row_number() > 0.1 * n())
})

#combine into single list by column (which represents each node)
ln.pc.ln.c.node.states.combined.cols <- bind_rows(ln.pc.ln.c.node.states.burnin.filtered)
```

```{r Calculate Mean and 95 CI for each node}
# Convert to long format
ln.pc.ln.c.node.states.combined.long <- ln.pc.ln.c.node.states.combined.cols %>%
  pivot_longer(
    cols = everything(),
    names_to = "node",
    values_to = "estimate"
...
#calculate mean, sd and CI for each node
ln.pc.ln.c.node.states.combined.long.summary <- 
  ln.pc.ln.c.node.states.combined.long%>%
  group_by(node) %>%
  summarise(
    mean = mean(estimate, na.rm = TRUE),
    median = median(estimate, na.rm = TRUE),
...
ln.pc.ln.c.node.states.combined.long.summary$node <- gsub("^X", "", ln.pc.ln.c.node.states.combined.long.summary$node)

#write to csv file and save
write.csv(ln.pc.ln.c.node.states.combined.long.summary, "ln_pc_ln_c_node_estimates_mcmc.csv")
```
...
#access the file paths
file_paths <- paste0("Node and Parameter Estimates/ln[Length]-ln[Width]/ln_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_model_params.txt")
...
```{r Importing Node State Estimates for ln[Length]-ln[Width]}

#define seed values used in the model fitting (part of filename)
seed_values <- c(1931, 1971, 1970, 1996, 2016)

files_ln_l_ln_w <- paste0("Node and Parameter Estimates/ln[Length]-ln[Width]/ln_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")

# Read all files into a list
ln.length.ln.width.node.states <- lapply(files_ln_l_ln_w, read.table, header = TRUE)

# If you want to assign each dataframe to its own variable, you can do so like this:
names(ln.length.ln.width.node.states) <- paste0("ln.length.ln.width.node.states.", 1:5, ".param")
list2env(ln.length.ln.width.node.states, envir = .GlobalEnv)

#filter first 10% of estimated node states which corresponds to burnin 
ln.length.ln.width.node.states.burnin.filtered <- lapply(ln.length.ln.width.node.states, function(df) {
  df %>% filter(row_number() > 0.1 * n())
})

#combine into single list by column (which represents each node)
ln.length.ln.width.node.states.combined.cols <- bind_rows(ln.length.ln.width.node.states.burnin.filtered)
```

```{r Calculate Mean and 95 CI for each node}
# Convert to long format
ln.length.ln.width.node.states.combined.long <- ln.length.ln.width.node.states.combined.cols %>%
  pivot_longer(
    cols = everything(),
    names_to = "node",
    values_to = "estimate"
...
#calculate mean, sd and CI for each node
ln.length.ln.width.node.states.combined.long.summary <- 
  ln.length.ln.width.node.states.combined.long%>%
  group_by(node) %>%
  summarise(
    mean = mean(estimate, na.rm = TRUE),
    median = median(estimate, na.rm = TRUE),
...
ln.length.ln.width.node.states.combined.long.summary$node <- gsub("^X", "", ln.length.ln.width.node.states.combined.long.summary$node)

#write to csv file and save
write.csv(ln.length.ln.width.node.states.combined.long.summary, "ln_length_ln_depth_node_estimates_mcmc.csv")
```

# Cranial/Anterior Elongation
...
#access the file paths
file_paths <- paste0("Node and Parameter Estimates/ln[Anterior]-ln[Width]/ln_ant_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_model_params.txt")
...
```{r Importing Node State Estimates for ln[Anterior Length]-ln[Width]}

#define seed values used in the model fitting (part of filename)
seed_values <- c(1931, 1971, 1970, 1996, 2016)

files_ln_ant_ln_w <- paste0("Node and Parameter Estimates/ln[Anterior]-ln[Width]/ln_ant_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")

# Read all files into a list
ln.ant.ln.width.node.states <- lapply(files_ln_ant_ln_w, read.table, header = TRUE)

# If you want to assign each dataframe to its own variable, you can do so like this:
names(ln.ant.ln.width.node.states) <- paste0("ln.ant.ln.width.node.states.", 1:5, ".param")
list2env(ln.ant.ln.width.node.states, envir = .GlobalEnv)

#filter first 10% of estimated node states which corresponds to burnin 
ln.ant.ln.width.node.states.burnin.filtered <- lapply(ln.ant.ln.width.node.states, function(df) {
  df %>% filter(row_number() > 0.1 * n())
})

#combine into single list by column (which represents each node)
ln.ant.ln.width.node.states.combined.cols <- bind_rows(ln.ant.ln.width.node.states.burnin.filtered)
```

```{r Calculate Mean and 95 CI for each node}
# Convert to long format
ln.ant.ln.width.node.states.combined.long <- ln.ant.ln.width.node.states.combined.cols %>%
  pivot_longer(
    cols = everything(),
    names_to = "node",
    values_to = "estimate"
...
#calculate mean, sd and CI for each node
ln.ant.ln.width.node.states.combined.long.summary <- 
  ln.ant.ln.width.node.states.combined.long%>%
  group_by(node) %>%
  summarise(
    mean = mean(estimate, na.rm = TRUE),
    median = median(estimate, na.rm = TRUE),
...
ln.ant.ln.width.node.states.combined.long.summary$node <- gsub("^X", "", ln.ant.ln.width.node.states.combined.long.summary$node)

#write to csv file and save
write.csv(ln.ant.ln.width.node.states.combined.long.summary, "ln_ant_ln_depth_node_estimates_mcmc.csv")
```


# Postcranial (Posterior) Elongation
...
#access the file paths
file_paths <- paste0("Node and Parameter Estimates/ln[posterior]-ln[Width]/ln_post_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_model_params.txt")
...
ln_posterior_ln_width_params_summary 
```
```{r Importing Node State Estimates for ln[Posterior Length]-ln[Width]}

#define seed values used in the model fitting (part of filename)
seed_values <- c(1931, 1971, 1970, 1996, 2016)

files_ln_post_ln_w <- paste0("Node and Parameter Estimates/ln[Posterior]-ln[Width]/ln_post_length_ln_width_", 1:5, "_mcmc_2e+6_BM_output_", seed_values, "_nodestates.txt")

# Read all files into a list
ln.post.ln.width.node.states <- lapply(files_ln_post_ln_w, read.table, header = TRUE)

# If you wpost to assign each dataframe to its own variable, you can do so like this:
names(ln.post.ln.width.node.states) <- paste0("ln.post.ln.width.node.states.", 1:5, ".param")
list2env(ln.post.ln.width.node.states, envir = .GlobalEnv)

#filter first 10% of estimated node states which corresponds to burnin 
ln.post.ln.width.node.states.burnin.filtered <- lapply(ln.post.ln.width.node.states, function(df) {
  df %>% filter(row_number() > 0.1 * n())
})

#combine into single list by column (which represents each node)
ln.post.ln.width.node.states.combined.cols <- bind_rows(ln.post.ln.width.node.states.burnin.filtered)
```

```{r Calculate Mean and 95 CI for each node}
# Convert to long format
ln.post.ln.width.node.states.combined.long <- ln.post.ln.width.node.states.combined.cols %>%
  pivot_longer(
    cols = everything(),
    names_to = "node",
    values_to = "estimate"
...
#calculate mean, sd and CI for each node
ln.post.ln.width.node.states.combined.long.summary <- 
  ln.post.ln.width.node.states.combined.long%>%
  group_by(node) %>%
  summarise(
    mean = mean(estimate, na.rm = TRUE),
    median = median(estimate, na.rm = TRUE),
...
ln.post.ln.width.node.states.combined.long.summary$node <- gsub("^X", "", ln.post.ln.width.node.states.combined.long.summary$node)

#write to csv file and save
write.csv(ln.post.ln.width.node.states.combined.long.summary, "ln_post_ln_depth_node_estimates_mcmc.csv")
```
